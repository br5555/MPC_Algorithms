#include "Faster_mpc_solver.h"



Faster_mpc_solver::Faster_mpc_solver(Matrix<double, num_state_variables, num_state_variables> A, Matrix<double, num_state_variables, num_manipulated_variables>  B,
...
Matrix<double, 2 * mpc_control_horizon, 1> Faster_mpc_solver::Evaluate(Matrix<double, 2 * mpc_control_horizon, 1>   x) {
	...
	for (int iter = 0; iter < max_iter; iter++) {
		...
		change_x = -0.6 * (Jacobian.transpose() * Jacobian + alfa * Eigen::MatrixXd::Identity(2 * control_horizon, 2 * control_horizon)).inverse() * Jacobian.transpose() *residuum;
		x = (x + change_x).eval();
...
Faster_mpc_solver::~Faster_mpc_solver()
{
}

Matrix<double, 2 * mpc_control_horizon, 1> Faster_mpc_solver::check_bounderies(Matrix<double, 2 * mpc_control_horizon, 1>   x) {
	dummy_u = u_current;
